...
method="duet_first_cycle_prior_context_transformer_prior_residual60_visda_proxy25_seed${seed}"
...
case "$run_dir" in
  output/uda/VISDA-C/TV/duet_first_cycle_prior_context_transformer_prior_residual60_visda_proxy25_seed2020) ;;
  *) echo "Refusing unexpected output path: $run_dir" >&2; exit 1 ;;
esac
rm -rf -- "$run_dir"

echo "==> Resume the exact Cycle-1 checkpoint; execute Cycle 2 only"
echo "==> Learn DUET-fallback keep vs challenger switch; restore the GT-free empirical prior"
echo "==> Rank 60% of conflicts, but change each branch only on an actual residual correction"
python image_target_of_oh_vs.py \
  ...
  DUET_CONTEXT.ANCHORS_PER_CLASS 8 \
  DUET_CONTEXT.ADAPTIVE_ANCHORS_ENABLED True \
  DUET_CONTEXT.MAX_ANCHORS_PER_CLASS 128 \
  ...
  DUET_CONTEXT.COMPARATOR_COVERAGE_FRACTION 0.60 \
  DUET_CONTEXT.CONFLICT_MEMORY_ENABLED False \
  DUET_CONTEXT.RELIABILITY_GATE_ENABLED True \
  DUET_CONTEXT.RELIABILITY_GATE_COVERAGE_FRACTION 0.60 \
  ...
  DUET_CONTEXT.TRANSITION_RESIDUAL_FALLBACK_ENABLED True \
  DUET_CONTEXT.TRANSITION_PRIOR_CALIBRATION_ENABLED True \
  DUET_CONTEXT.TRANSITION_PRIOR_STRENGTH 1.0 \
  DUET_CONTEXT.TRANSITION_PRIOR_SMOOTHING 1.0 \
  ...
  DUET_CONTEXT.TRANSITION_SYNTHETIC_MIX_FRACTION 0.0 \
...
grep -q "DUET delayed real-conflict supervision: cycle=2;.*candidate_semantics=duet_fallback_keep_vs_challenger_switch" "$log_file"
grep -q "DUET transition comparator training: cycle=2;.*empirical_prior_restored_at_inference=True" "$log_file"
grep -q "DUET transition-comparator fusion: cycle=2;.*residual_fallback=True;.*coverage=60" "$log_file"
grep -q "DUET reliability-gated comparator: cycle=2;.*coverage=60.*residual_corrections=" "$log_file"
grep -q "DUET reliability-gate isolation: cycle=2;.*kl_soft=residual_vs_clip_teacher;.*confi_dis=residual_vs_duet_fallback" "$log_file"
if grep -q "DUET context transformer admitted: cycle=2" "$log_file"; then
  echo "Prior-residual run unexpectedly changed hard admission" >&2
  exit 1
fi

echo "==> Prior-calibrated DUET residual result"
