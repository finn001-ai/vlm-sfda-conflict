...
audit_dir="${base_dir}/agreement_confidence_weight_audit"

for path in "$snapshot" "$source_lock" "$target_list" "$class_names"; do
  if [ ! -f "$path" ]; then
    echo "Missing agreement confidence-weight audit input: $path" >&2
    echo "Run first: bash tools/run_visda_cycle2_conflict_memory_audit.sh" >&2
    exit 1
  fi
done

echo "==> CPU-only cycle-1 DUET agreement confidence-weight audit"
echo "==> Keeps every agreement and preserves mean CE weight within each pseudo class"
echo "==> Compares continuous CLIP-confidence weight with unweighted CE and bottom-10 delay"
echo "==> No image/model/checkpoint load, forward, backward, optimizer, or training"

python tools/audit_visda_agreement_confidence_weight.py \
  --snapshot "$snapshot" \
...
echo "==> Audit complete: ${audit_dir}/visda_agreement_confidence_weight_summary.json"
