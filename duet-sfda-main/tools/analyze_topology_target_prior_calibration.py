#!/usr/bin/env python
"""Probe graph-target distribution alignment before adaptation training.

Ground-truth labels are used only for reporting. The tested method uses the
dual-space graph posterior mean as a smoothed target class prior:

    q(y|x) is proportional to p(y|x) * (pi_target(y) / pi_teacher(y))^gamma

This is class-prior alignment, not a per-sample conflict selector.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out",
        default="output/uda/office-home/topology_target_prior_probe.json",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=-1.0,
        help="Graph/uniform target-prior mix. Negative uses entropy-adaptive mixing.",
    )
...
def analyze(path: Path, args: argparse.Namespace) -> dict[str, int | float | str | bool]:
    ...
    _, _, inverse_mix, graph_prior, anchors = topology_prior_calibrate(
        task_features,
        clip_features,
        source_prob,
        clip_prob,
    ...
    target_acc = accuracy(target_mix_prob, labels)
    target_delta = round(target_acc - both_acc, 4)
    inverse_acc = accuracy(inverse_mix, labels)
    anchor_correct = source_label[anchors] == labels[anchors]
    ...
    return {
        ...
        "inverse_topo_prior_mix_accuracy": inverse_acc,
        "inverse_topo_minus_both_prior": round(inverse_acc - both_acc, 4),
        "topo_target_prior_source_accuracy": accuracy(target_source, labels),
        "topo_target_prior_clip_accuracy": accuracy(target_clip, labels),
        "topo_target_prior_mix_accuracy": target_acc,
        "topo_target_minus_both_prior": target_delta,
        "raw_agreement_rate": agreement_rate(source_prob, clip_prob),
        "both_prior_agreement_rate": agreement_rate(both_source, both_clip),
        "topo_target_prior_agreement_rate": agreement_rate(target_source, target_clip),
        ...
        "target_prior_min": round(float(target_prior.min().item()), 6),
        "target_prior_max": round(float(target_prior.max().item()), 6),
        "target_prior_entropy": entropy(target_prior),
        "resolved_target_mix": round(float(resolved_target_mix), 6),
        "pass_task_gate": bool(target_delta >= args.min_task_delta),
    }
...
def main() -> None:
    ...
    mean_delta = round(float(np.mean([task["topo_target_minus_both_prior"] for task in tasks])), 4)
    output = {
        "config": vars(args),
        ...
        "reason": (
            "Topology-target alignment beats both_prior on enough probe tasks."
            if pass_tasks >= args.min_pass_tasks and mean_delta > 0
            else "Topology-target alignment does not beat both_prior strongly enough."
        ),
        "pass_tasks": int(pass_tasks),
        "mean_topo_target_minus_both_prior": mean_delta,
        "tasks": tasks,
    }
