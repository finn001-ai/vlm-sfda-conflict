...
method="duet_first_cycle_prior_context_transformer_adaptive_anchor_overlap80_visda_proxy25_seed${seed}"
...
case "$run_dir" in
  output/uda/VISDA-C/TV/duet_first_cycle_prior_context_transformer_adaptive_anchor_overlap80_visda_proxy25_seed2020) ;;
...
python image_target_of_oh_vs.py \
  ...
  DUET_CONTEXT.RELIABILITY_GATE_LOSS_WEIGHT 0.20 \
...
grep -q "adaptive_anchors=True" "$log_file"
grep -q "DUET context refinement: cycle=2;.*anchor_capacities=\[" "$log_file"
...
echo "==> Adaptive-anchor temporal-overlap result"
grep "DUET context refinement:" "$log_file"
grep "DUET delayed real-conflict supervision:" "$log_file"
grep "DUET temporal transition eligibility:" "$log_file"
