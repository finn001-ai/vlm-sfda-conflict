...
seed=2020
method="plmatch_visda_feature_gravity_audit_seed${seed}"
run_dir="output/uda/VISDA-C/TV/${method}"
audit_dir="${run_dir}/feature_gravity_audit"
summary="${audit_dir}/visda_conflict_feature_gravity_summary.json"
...
echo "==> Full target, seed=${seed}; no optimizer, no adaptation, no checkpoints"
python tools/audit_visda_conflict_feature_gravity.py \
...
for artifact in \
  "${audit_dir}/visda_conflict_feature_gravity_signals.csv" \
  "${audit_dir}/visda_conflict_feature_gravity_signals.npz" \
  "${audit_dir}/visda_conflict_feature_gravity_signal_lock.json" \
  "${audit_dir}/visda_conflict_feature_gravity_oracle_diagnostic.csv" \
  "${audit_dir}/visda_conflict_feature_gravity_classwise.csv" \
  "$summary"; do
  if [ ! -s "$artifact" ]; then
...
echo "==> This command never authorizes or starts training"
