...
# VisDA-C Delayed Agreement Credit on the locked class-proportional 25%
...
method_name="duet_delayed_agreement_credit_visda_proxy25_seed${experiment_seed}"
...
echo "==> DAC starts before epoch 1; old DUET Cycle-1 checkpoints are intentionally not reused"
echo "==> No PoE, fixed initial anchor, CSM, Comparator, target GT, or third visual model"
...
if ! grep -q "soft_coverage=100.00%; conflict_hard_coverage=100.00%" "$latest_log"; then
  echo "Run did not execute the full-coverage DAC path" >&2
  exit 1
fi
if ! grep -q "entropy_poe=False; csm=False" "$latest_log"; then
  echo "Run did not execute the intended non-COSMO teacher construction" >&2
  exit 1
fi

echo "==> Final five epochs"
grep "DUET delayed agreement credit epoch:" "$latest_log" | tail -n 5
