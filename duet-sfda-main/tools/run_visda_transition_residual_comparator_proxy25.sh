...
method="duet_first_cycle_prior_context_transformer_transition_residual80_visda_proxy25_seed${seed}"
...
case "$run_dir" in
  output/uda/VISDA-C/TV/duet_first_cycle_prior_context_transformer_transition_residual80_visda_proxy25_seed2020) ;;
  *) echo "Refusing unexpected output path: $run_dir" >&2; exit 1 ;;
esac
rm -rf -- "$run_dir"

echo "==> Resume the exact Cycle-1 checkpoint; execute Cycle 2 only"
echo "==> Keep all matured conflicts, balance training batches, inject only the CLIP-KL residual"
# RELIABILITY_GATE_LOSS_WEIGHT matches ACTIVE.KL_PAR (0.40), making the
# residual a reliability-weighted interpolation of the existing CLIP teacher.
...
grep -q "sampling=direction_balanced_without_downsampling" "$log_file"
grep -q "loss_mode=clip_kl_residual" "$log_file"
...
if grep -q "DUET context transformer admitted: cycle=2" "$log_file"; then
  echo "Transition residual method unexpectedly changed hard admission" >&2
  exit 1
fi

echo "==> Transition-residual result"
