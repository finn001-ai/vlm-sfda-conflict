...
echo "==> VisDA-C temporal-stability full run: PL/GTR stable cycles 3, 8 cycles, seed 2020"
...
python tools/extract_final_accuracy.py \
  ...
  --selection peak \
  > "$result_dir/temporal_precision_head_visda_stability3_seed2020_accuracy.csv"

python tools/summarize_visda_temporal_precision_head.py \
  --glob "output/uda/VISDA-C/TV/${method}/*.txt" \
  --out "$result_dir/temporal_precision_head_visda_stability3_seed2020_summary.json" \
  --csv-out "$result_dir/temporal_precision_head_visda_stability3_seed2020_per_class.csv" \
...
python tools/analyze_temporal_conflict_dynamics.py \
  ...
  --out "$result_dir/temporal_precision_head_visda_stability3_seed2020_dynamics.json" \
  --min-pass-tasks 1
