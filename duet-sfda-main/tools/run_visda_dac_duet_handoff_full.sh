...
#   2. hand its Task F/B state to the released DUET path for four cycles;
#   3. retain the frozen source classifier C to preserve class geometry.
# This gives 15 + 4*4 = 31 target passes, below DUET's 8*4 = 32.
...
echo "==> Stage 1 reused: DAC F/B final checkpoint, 15 target passes"
echo "==> Source classifier C retained and frozen; DAC target_C is not used"
