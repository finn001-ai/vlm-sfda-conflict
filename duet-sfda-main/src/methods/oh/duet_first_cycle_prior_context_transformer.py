...
This file is a self-contained copy of the DUET training loop
(``src/methods/oh/plmatch.py``) plus the new candidate
``duet_first_cycle_prior_context_transformer``.  It intentionally does NOT
modify ``plmatch.py`` / ``plmatch_clean.py``:  the complete training logic
lives here so the candidate can be audited standalone.

Core idea (strictly built on ``duet_first_cycle_prior``):

1.  Build a class-balanced anchor bank from Task/CLIP-consistent,
    high-confidence target samples (hard pseudo-labels, no ground-truth).
2.  Treat Task/CLIP strict conflicts and low-confidence weak agreements as
    image-level feature queries.
3.  A lightweight Cross-Attention Transformer (or cosine-kNN / prototype
    control) re-classifies queries against the anchor context; decision rules
    then admit (support Task / support CLIP / third class), keep the original
    agreement rule, or abstain.

When ``DUET_CONTEXT.ENABLED=False`` (or the cycle is outside
``DUET_CONTEXT.ACTIVE_CYCLES``) every path degenerates exactly to the
original ``duet_first_cycle_prior`` behavior.
...
def train_target(
    ...
    *,
    ...
    """训练原始 DUET-FCP，或启用一个明确隔离的候选改动。

    ``first_cycle_prior=False`` 是发布版 DUET 的原始路径。DUET-FCP 入口只把
    该参数设为 ``True``；stable memory、target head、graph teacher 和 GTR
    均不在本文件中，因此不会混入候选。

    ``context_conflict_transformer=True`` 是本文件的新候选
    ``duet_first_cycle_prior_context_transformer``：它必须与
    ``first_cycle_prior=True`` 同时启用，并且
    ``cfg.DUET_CONTEXT.ENABLED=True``。启用时，在第 1 个 cycle 用
    Task/CLIP 高置信一致样本构建 class-balanced anchor bank，用轻量
    Cross-Attention Transformer 对 strict conflict / weak agreement 查询
    做确认、纠正或拒绝；``cfg.DUET_CONTEXT.REFINER_TYPE`` 可切换为
    cosine_knn / prototype 对照。关闭（ENABLED=False 或不在
    ACTIVE_CYCLES 中）时完全退化为原始 duet_first_cycle_prior。
    ...
    # --- Candidate isolation: context_conflict_transformer -----------------
    # It is the only candidate allowed to combine with first_cycle_prior
    # (because it is strictly built on top of DUET-FCP).  Every other
    # candidate stays exclusive.
    context_requested = bool(context_conflict_transformer)
    if context_requested and not first_cycle_prior:
        raise ValueError(
            "context-conflict transformer requires first_cycle_prior=True"
        )
    if context_requested and any(
        (
            boundary_router,
            attribute_reliability_kl,
            support_conditioned_clip,
        ...
        raise ValueError(
            "context-conflict transformer cannot be combined with other "
            "DUET candidates"
        )
    context_enabled = bool(cfg.DUET_CONTEXT.ENABLED) if context_requested else False
    context_active_cycles = tuple(
        int(v) for v in cfg.DUET_CONTEXT.ACTIVE_CYCLES
    )
    context_refiner = str(cfg.DUET_CONTEXT.REFINER_TYPE)
    if context_enabled and context_refiner not in ("transformer", "cosine_knn", "prototype"):
        raise ValueError(
    ...
    logging.info(
        "DUET context transformer: requested={}; enabled={}; "
        "refiner={}; active_cycles={}; anchors_per_class={}; "
        "anchor_task_conf={:.2f}; anchor_clip_conf={:.2f}; "
        "strict_conflict={}; weak_agreement={}; "
    ...
    while curr_cycle < cfg.ACTIVE.CYCLE:
        ...
        label_result = obtain_label(
            ...
            context_feature_dim=int(cfg.bottleneck),
...
def obtain_label(
    ...
    if context_active:
        if context_cfg is None or context_transformer is None or context_optimizer is None:
            raise ValueError(
                "context-conflict transformer requires cfg, module and optimizer"
            )
        # Transformer 只在 no_grad 块外训练；这里只收集 detached features。
        collect_features = True
        collect_sample_indices = True
    ...
    if context_active and first_cycle_prior:
    ...
    if context_active:
        ...
        # weak-agreement 未通过验证的样本暂缓进入硬 CE（admission_matching=False），
        # 但仍参与原始 consistency 与 CLIP soft KL（kl_soft 保持 clip 概率）。
    ...
    if context_payload is not None:
        # 成功 resolved 的 strict conflict 必须同时修改三个对象：
    ...
    if context_payload is not None:
        # 先生成原始 DUET 混合分布，再覆盖 resolved 行：mem_label 随之变为
        # context_top1，与 kl_soft 的 refined target 保持一致。
