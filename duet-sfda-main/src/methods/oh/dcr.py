...
def train_target(cfg):
    """Run the stable DCR conflict-refinement stage."""
    ...
    method_name = str(getattr(model_cfg, "METHOD", ""))
    handoff_mode = method_name == "dcr" or method_name.startswith("dcr_")
    handoff_cfg = getattr(cfg, "DCR", None)
    ...
    if handoff_final_extra_epochs < 0:
        raise ValueError("DCR.FINAL_EXTRA_EPOCHS must be non-negative")
    if handoff_final_extra_epochs and not handoff_mode:
        raise ValueError(
            "DCR.FINAL_EXTRA_EPOCHS is only valid for DCR"
        )
    if credit_preserving and not handoff_mode:
        raise ValueError(
            "DCR.CREDIT_PRESERVING requires the DCR method"
        )
    ...
    if credit_preserving and not credit_state_path:
        raise ValueError(
            "DCR.STATE_PATH is required for credit preservation"
        )
    if not 0.0 <= credit_conflict_fraction <= 1.0:
        raise ValueError(
            "DCR.CONFLICT_HARD_FRACTION must be in [0, 1]"
        )
    if credit_soft_replacement_mode not in {
        ...
        raise ValueError(
            "DCR.SOFT_REPLACEMENT_MODE must be none, "
            "all_conflicts, or task_supported"
        )
    if credit_memory_write_mode not in {"locked", "writable", "frozen"}:
        raise ValueError(
            "DCR.MEMORY_WRITE_MODE must be locked, writable, "
            "or frozen"
        )
    if not 0.0 <= credit_decay < 1.0:
        raise ValueError("DCR.CREDIT_DECAY must be in [0, 1)")
    if credit_eta <= 0.0:
        raise ValueError("DCR.CREDIT_ETA must be positive")
    if not 0.0 < credit_memory_update_rate <= 1.0:
        raise ValueError(
            "DCR.MEMORY_UPDATE_RATE must be in (0, 1]"
        )
    if credit_mode not in {"delayed", "uniform"}:
        raise ValueError(
            "DCR.CREDIT_MODE must be delayed or uniform"
        )
    if credit_feedback_mode not in {
        ...
        raise ValueError(
            "DCR.FEEDBACK_MODE must be agreement_temporal "
            "or agreement_only"
        )
    ...
    if credit_preserving:
        if not osp.isfile(credit_state_path):
            raise FileNotFoundError(
                "DCR memory state does not exist: {}".format(
                    credit_state_path
                )
            )
        ...
        logging.info(
            "DCR memory state loaded: path={}; samples={}; classes={}; "
    ...
    while curr_cycle < cfg.ACTIVE.CYCLE:
        ...
        if credit_preserving and credit_freeze_clip:
            logging.info(
                "DCR VLM update: cycle={}; skipped=True; "
...
def obtain_label(
    ...
    if credit_runtime is not None:
        ...
        logging.info(
            "DCR memory teacher: cycle={}; agreements={}; "
    ...
    if credit_payload is not None:
        ...
        logging.info(
            "DCR asymmetric residual guidance: cycle={}; unresolved_replaced={}; "
