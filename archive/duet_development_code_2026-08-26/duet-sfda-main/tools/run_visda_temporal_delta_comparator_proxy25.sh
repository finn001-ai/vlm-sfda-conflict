...
method="duet_first_cycle_prior_context_transformer_temporal_delta80_visda_proxy25_seed${seed}"
...
case "$run_dir" in
  output/uda/VISDA-C/TV/duet_first_cycle_prior_context_transformer_temporal_delta80_visda_proxy25_seed2020) ;;
  *) echo "Refusing unexpected output path: $run_dir" >&2; exit 1 ;;
esac
rm -rf -- "$run_dir"

echo "==> Resume the exact Cycle-1 checkpoint; execute Cycle 2 only"
echo "==> Train mirrored A/B Comparator on post-minus-pre Cycle-1 evidence"
...
grep -q "feature_mode=temporal_delta_16D_with_A_B_mirroring" "$log_file"
grep -q "DUET temporal transition eligibility: cycle=2" "$log_file"
...
if grep -q "DUET context transformer admitted: cycle=2" "$log_file"; then
  echo "Temporal method unexpectedly changed hard admission" >&2
  exit 1
fi

echo "==> Temporal-delta result"
