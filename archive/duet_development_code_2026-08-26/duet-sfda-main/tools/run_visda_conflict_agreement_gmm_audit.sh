...
audit_dir="${base_dir}/agreement_gmm_audit"

for path in "$snapshot" "$source_lock" "$target_list" "$class_names"; do
  if [ ! -f "$path" ]; then
    echo "Missing agreement-GMM audit input: $path" >&2
    echo "Run first: bash tools/run_visda_cycle2_conflict_memory_audit.sh" >&2
    exit 1
  fi
done

echo "==> CPU-only VisDA agreement-conditioned joint-evidence GMM audit"
echo "==> References: 6,777 cycle-1 task/CLIP agreements"
echo "==> Queries: 7,070 task/CLIP conflicts; choices stay inside top-2 union"
echo "==> Evidence: task+CLIP centered log probabilities; no fitted threshold"
echo "==> No image/model/checkpoint load, forward, backward, optimizer, or training"

CUDA_VISIBLE_DEVICES="" python tools/audit_visda_conflict_agreement_gmm.py \
  --snapshot "$snapshot" \
...
echo "==> Audit complete: ${audit_dir}/visda_conflict_agreement_gmm_summary.json"
